...
// ---------------- FINAL PRICE SELECTION (CORRIGIDO: Menor peso para Total Calculado) ----------------
...
      else if (src.includes("selector")) score += 50;
      else if (src.includes("nearCTA")) score += 45;
      else if (src.includes("xhr")) score += 20;
      else if (src.includes("body")) score += 5;
      else score += 5;

      // CORREÇÃO 1: Redução do boost para Computed Totals (de 70 para 10)
      if (p.computedTotal) score += 10; 
      
      // Penalidade para o valor da parcela isolada
      if (p.isParcel) score -= 50;

      // CORREÇÃO 2: Penalizar "List Price" (Preço "De") e bonificar "Sale Price" (Preço "Por")
      if (field.includes("list") || field.includes("original") || field.includes("old") || field.includes("from")) {
...
  // CORREÇÃO 3: Desempate pelo menor preço (Prioriza 'À Vista')
  if (scored.length >= 2) {
    const second = scored[1];
    const scoreDiff = best.score - second.score;
    // Se o score for muito próximo (< 25 pontos), e o segundo candidato for
    // um preço significativamente menor (> 40% do melhor preço), preferir o menor.
    if (scoreDiff < 25 && second.num < best.num && second.num > (best.num * 0.4)) {
      debug.finalChoice = { chosen: second, reason: "lower_price_tiebreaker" };
      return `R$ ${Number(second.num).toFixed(2).replace(".", ",")}`;
    }
  }
